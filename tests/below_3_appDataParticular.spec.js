...
test.only('Login in Salesforce Org and Getting Data From AppExchange For a Particular App Where Rating Criteria is Below 3 Stars', async ({ browser }) => {
    ...
    // //check for first app
    await page.waitForTimeout(3000);
    const appName = await page.locator(".appx-tile-content-inner-2 .appx-txt-primary .appx-tile-title").nth(0).textContent();
    console.log(appName);


    //Hover on the App Title
    //await page.waitForTimeout(3000);
    await page.locator(".appx-tile-content-inner.appx-tile-content-inner-1 .appx-tile-img.appx-tile-img-brand .uiImage img").nth(0).hover();

    //click on the App Title
    await page.locator(".appx-tile-content-inner-2 span.appx-tile-descr").nth(0).click();
