...
test('Login in Salesforce Org and Getting Data From AppExchange For all App Where Rating Criteria is Below 3 Stars', async ({ browser }) => {
    ...
    // //check for first app
    // await page.waitForTimeout(3000);
    // const appName = await page.locator(".appx-tile-content-inner-2 .appx-txt-primary .appx-tile-title").nth(i).textContent();
    // console.log(appName);
    ...
    // // Getting the Text of all App Below 3 Star Rating.
    page.keyboard.down('End');
    ...
    for (let i = 0; i < p; ++i) {
        ...
        if (titleApp != "Limits for Salesforce") {
            if (titleApp != "FreshBooks for Salesforce") {
        ...
        //Getting Published Detail ------for LATEST RELAESE DATE
        ...
        for (let j = 0; j < a; j++) {
            page.keyboard.down('End');
            const user = await page.locator("article.slds-post.appx-post.appx-comment header .slds-media__body").nth(j).textContent();
            const desc = await page.locator("article.slds-post.appx-post.appx-comment div.slds-post__content.slds-text-longform.appx-comment-content").nth(j).textContent();
            const result = user + "\n" + desc;
            console.log(result);
        }
