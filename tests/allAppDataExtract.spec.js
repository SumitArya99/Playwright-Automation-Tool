...
test.only('Login in Salesforce Org and Getting Data From AppExchange For all App Where Rating Criteria is 1 Stars', async ({ browser }) => {
    ...
    await page.locator("input.slds-checkbox_faux[value='rt3']").click();
    ...
    await page.waitForTimeout(3000);
    const appName = await page.locator(".appx-tile-content-inner-2 .appx-txt-primary .appx-tile-title").nth(75).textContent();
    console.log(appName);
    ...
    for (let i = 86; i < 89; ++i) {
        ...
        if (titleApp != "Astrea File Helper") {
            if (titleApp != "ZakCalendarᴿ") {
                if (titleApp != "Astrea Attachment Converter") {
                    if (titleApp != "Opportunity Primary Contact Required") {
        ...
        //For Loading Review Data 20-20 At a Time and Page Down Process
        var a = 0;
        var b = user;
        a = a + b;
        page.keyboard.down('End');
